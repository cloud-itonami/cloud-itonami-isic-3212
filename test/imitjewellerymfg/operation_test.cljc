(ns imitjewellerymfg.operation-test
  "Smoke tests for the compiled ImitationJewelryOperationActor graph
  itself (build + one happy path per op). The governor's full rule
  contract (HARD holds, escalation, phase gating) is exercised in
  `imitjewellerymfg.governor-contract-test`; the Store contract in
  `imitjewellerymfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ImitationJewelryOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "casting-001" :severity :moderate :description "plating-bath fume concern"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
