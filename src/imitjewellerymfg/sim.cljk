(ns imitjewellerymfg.sim
  ...
  plating equipment directly (permanently blocked, no override), a
  proposal that tries to self-issue a materials-safety/lead-content
  compliance certification (permanently blocked, no override), a
  double-schedule of the same maintenance window, a production-batch
  patch with a fabricated base-metal-type, a production-batch patch
  with an implausible plating-thickness-microns reading, a production-
  batch patch with an implausible weight-grams reading, and a
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t1"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:base-metal-type :zinc-alloy :last-assessed "2026-07-14"}}
                       coordinator))
    ...
    (println "== schedule-maintenance mnt-2 on plating-002 (UNVERIFIED/unregistered plating station -> HARD hold) ==")
    ...
    (println "== log-production-batch batch-001 with a fabricated base-metal-type -> HARD hold ==")
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:base-metal-type :unobtainium}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible plating-thickness-microns -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:plating-thickness-microns 500.0}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a materials-safety/lead-content compliance certification -> HARD hold, PERMANENT ==")
