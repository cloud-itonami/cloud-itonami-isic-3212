(ns imitjewellerymfg.advisor
  "ImitationJewelryAdvisor -- the *contained intelligence node* for the
  imitation-jewellery-workshop plant-operations coordination actor.

  It normalizes production-batch patches (base-metal-type/plating-
  thickness-microns/weight-grams/defect-rate), drafts a casting/
  plating-equipment maintenance scheduling proposal against a piece of
  equipment, drafts a safety-concern flag, and drafts an outbound
  imitation-jewellery shipment coordination proposal against a
  ...
  a committed record and NEVER a real casting/plating-equipment
  actuation, freight dispatch, or materials-safety/lead-content
  compliance certification -- see README `What this actor does NOT
  do`. Every output is censored downstream by `imitjewellerymfg.governor`
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `imitjewellerymfg.governor`)
     ...
     :effect     kw             ; how a commit would mutate the SSoT --
                                 ...
                                 ; effects, NEVER a direct casting/
  ...
  `imitjewellerymfg.governor` HARD-holds any request that doesn't, so a
  mis-wired caller can never reach a commit path even if this advisor
  were compromised."
  (:require #?(:clj  [clojure.edn :as edn]
               :cljs [cljs.reader :as edn])
            [clojure.string :as str]
            [imitjewellerymfg.registry :as registry]
            [imitjewellerymfg.store :as store]
            [langchain.model :as model]))

(defn- log-production-batch
  "Production-batch intake upsert -- the advisor only normalizes/
  validates the patch; it does not invent the batch's base-metal-type,
  plating-thickness-microns, weight-grams, or defect-rate, nor its
...
(defn- schedule-maintenance
  "Draft a casting/plating-equipment maintenance-window scheduling
  ...
  `imitjewellerymfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a materials-safety(lead/cadmium-content-compliance/plating-
  chemical)/theft-security concern. ALWAYS `:stake :coordination/
  safety-concern` -- a safety concern is NEVER a proposal the advisor
  may quietly downgrade to low-stakes, and it is never gated on the
  ...
  on an administrative technicality). See `imitjewellerymfg.phase`: no
  phase ever adds this op to a phase's `:auto` set;
  `imitjewellerymfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound imitation-jewellery shipment coordination
  ...
  `imitjewellerymfg.governor` NEVER trusts it: it independently
...
(def ^:private system-prompt
  (str "You are the advisor for an imitation-jewellery workshop "
       ...
       "equipment or batches. Never propose direct actuation of casting/"
       "plating equipment (this actor only proposes, it never executes). "
       "Never propose self-issuing a materials-safety/lead-content "
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `imitjewellerymfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :imitjewellerymfg-advisor-proposal
   :op         (:op request)
