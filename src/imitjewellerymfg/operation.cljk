(ns imitjewellerymfg.operation
  "ImitationJewelryOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (ImitationJewelryAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Imitation Jewellery Workshop Plant Operations Governor (:govern) and
  ...
  each operation is auditable and checkpointed. A workshop's lifecycle
  ...
  decision to a human workshop supervisor/shipping approver. The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [imitjewellerymfg.advisor :as advisor]
...
(defn build
  "Compiles an ImitationJewelryOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `imitjewellerymfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ImitationJewelryAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Imitation Jewellery Workshop Plant Operations Governor -- independent censor.
      (g/add-node :govern
